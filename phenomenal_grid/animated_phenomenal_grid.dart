...
import 'package:iosite/_packages/phenomenal_grid/phenomenal_grid.dart';
import 'package:iosite/_packages/phenomenal_grid/phenomenal_grid_animation_delegate.dart';
...
import 'package:iosite/_packages/flutter_spice_powder/common_data_types_powder/string.dart';
import 'package:iosite/_packages/flutter_spice_powder/dev/values.dart';
...
class AnimatedPhenomenalGrid extends StatelessWidget {
  ...
  AnimatedPhenomenalGrid.list({
    ...
    layout = layout ?? PhenomenalGridLayout.defaultLayout(),
    animationDelegate = animationDelegate ?? PhenomenalGridAnimationDelegate.magnetic(),
    buildItem = ((context, itemInfo, status) => children.hasIndex(itemInfo.itemKey) ? children[itemInfo.itemKey].buildItem(context, itemInfo, status) : EmptyWidget())
  {
    _calculateRenderInfo(); 
  }
    ...
    for (OrderedItemInfo fiItem in layout.orderedItems)
    {
      ...
      _RenderInfo renderInfo = _RenderInfo(
        animationInfo: AnimationInfo(
          delay: disableAnimations.thenEither(1.milliseconds, _calculateDelay(previousItem, previousNoneIndependentItem, previousIndependentItem, animationTiming)), 
